...
const Members: React.FC = () => {
  ...
  const [memberTypeFilter, setMemberTypeFilter] = useState<'all' | 'fulltime' | 'onetime'>('all');
  ...
  const handleAddMember = (memberData: MemberFormData) => {
    ...
    loadMembers();
    setShowForm(false);
    showNotification(`${newMember.firstName} ${newMember.lastName} has been added successfully!`, 'success');
  };
  ...
  const handleEditMember = (memberData: MemberFormData) => {
    if (editingMember) {
      ...
      if (updatedMember) {
        ...
        showNotification(`${updatedMember.firstName} ${updatedMember.lastName} has been updated successfully!`, 'success');
      } else {
  ...
  const handleDeactivateMember = (member: Member) => {
    setConfirmModal({
      ...
      onConfirm: () => {
        ...
        loadMembers();
        closeModal();
        showNotification(`${member.firstName} ${member.lastName} has been deactivated.`, 'info');
      },
      isDangerous: false
    });
  };
  ...
  const handleReactivateMember = (member: Member) => {
    ...
    loadMembers();
    showNotification(`${member.firstName} ${member.lastName} has been reactivated.`, 'success');
  };
  ...
  const handleDeleteMember = (member: Member) => {
    setConfirmModal({
      ...
      onConfirm: () => {
        ...
        loadMembers();
        closeModal();
        showNotification(`${member.firstName} ${member.lastName} has been permanently deleted.`, 'warning');
      },
      isDangerous: true
    });
  };
      ...
      <div className="search-section">
        ...
        <div className="filter-section">
          ...
          <select
            ...
            onChange={(e) => setMemberTypeFilter(e.target.value as 'all' | 'fulltime' | 'onetime')}
          ...
          return (
            <div key={member.id} className={`member-card ${!member.isActive ? 'inactive' : ''}`}>
              ...
              <div className="member-info">
                ...
                <div className="info-item">
                  ...
                  <span className={`member-type ${member.memberType}`}>
                    {member.memberType === 'fulltime' ? 'Fulltime Member' : 'One Time Member'}
                  </span>
                </div>
                <div className="info-item">
              ...
              <div className="emergency-contact">
                ...
                <div className="info-item">
                  <strong>Phone:</strong> {formatPhone(member.emergencyContact.phone)}
                </div>
                <div className="info-item">
                  <strong>Relationship:</strong> {member.emergencyContact.relationship}
                </div>
              </div>
              ...
              <div className="membership-info">
                ...
                {membership ? (
                  <div>
                    ...
                    <div className="info-item">
                      <strong>Monthly Amount:</strong> ${membership.monthlyAmount}
                    </div>
                    <div className="info-item">
                      <strong>Renewal Date:</strong> {formatDate(membership.renewalDate)}
                    </div>
                  </div>
                ) : (
                  <div>
